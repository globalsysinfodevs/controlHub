...
export function ActivateAccountPage() {
  ...
  useEffect(() => {
    if (!token) {
      setTokenError("No se encontró el token de activación en la URL.");
      setValidating(false);
      return;
    }
    authApi
      ...
      .catch((e: Error) => {
        setTokenError(e.message ?? "El token no es válido o ha expirado.");
        setValidating(false);
      });
  }, [token]);
    ...
    <div className="flex min-h-screen items-center justify-center bg-g-light px-4">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 flex flex-col items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-primary">Configura tu contraseña</h1>
          <p className="text-center text-sm text-g-dark">
            Establece una contraseña segura para activar tu cuenta de administrador
          </p>
        </div>
        ...
        <div className="panel p-8">
          ...
          {validating && (
            <div className="flex flex-col items-center gap-3 py-8">
              ...
              <p className="text-sm text-g-dark">Verificando invitación…</p>
            </div>
          )}
